...
#Configure the radius database and import data
function set_mysql2() {
	systemctl restart mariadb
	sleep 3
	mysqladmin -u root password ""${sqladmin}""
	mysql -uroot -p${sqladmin} -e "create database radius;"
	mysql -uroot -p${sqladmin} -e "grant all privileges on radius.* to radius@localhost identified by 'p0radius0p_iVPN';"
	mysql -uradius -p'p0radius0p_iVPN' radius < /etc/raddb/mods-config/sql/main/mysql/schema.sql  
	systemctl restart mariadb
}
...
function set_freeradius3(){
	...
	#Find the string, remove the first letter of the comment #
	...
	sed -i 's/password = "radpass"/password = "p0radius0p_iVPN"/g' /etc/raddb/mods-available/sql	
...
function set_daloradius4(){
	...
	mysql -uradius -p'p0radius0p_iVPN' radius < contrib/db/fr2-mysql-daloradius-and-freeradius.sql
	mysql -uradius -p'p0radius0p_iVPN' radius < contrib/db/mysql-daloradius.sql
	sleep 3
	sed -i "s/\['CONFIG_DB_USER'\] = 'root'/\['CONFIG_DB_USER'\] = 'radius'/g"  /var/www/html/daloradius/library/daloradius.conf.php
	sed -i "s/\['CONFIG_DB_PASS'\] = ''/\['CONFIG_DB_PASS'\] = 'p0radius0p_iVPN'/g" /var/www/html/daloradius/library/daloradius.conf.php
...
function set_fix_radacct_table5(){
	...
	mysql -uradius -p'p0radius0p_iVPN' radius < /tmp/radacct_new.sql
...
<VirtualHost *:9090>
 ...
 ServerName daloradius.devsmine.com
...
rm -rf dbback.tar.gz
echo 'mysql -uradius -pp0radius0p_iVPN -e "UPDATE radius.radacct SET acctstoptime = acctstarttime + acctsessiontime WHERE ((UNIX_TIMESTAMP(acctstarttime) + acctsessiontime + 240 - UNIX_TIMESTAMP())<0) AND acctstoptime IS NULL;"' >> /usr/mysys/clearsession.sh
...
echo "==========================================================================
				 ...
				 The following information will be automatically saved to the /root/info.txt file	
		          ...
		          VPN account management background address：http://$public_ip:9090
		                            account number：administrator password:radius
		                             
		          If you use radius authentication, you need to comment the password authentication line in the /etc/ocserv/ocserv.conf file.
			   auth = "plain[passwd=/etc/ocserv/ocpasswd]"
			   #The following method is to use radius to authenticate users. If using radius, please note the password verification above.
			   #auth = "radius[config=/etc/radiusclient-ng/radiusclient.conf,groupconfig=true]"
			   #After adding the following sentence, daloradius online users can see that the user is online
			   #acct = "radius[config=/etc/radiusclient-ng/radiusclient.conf]"
			   After the modification is completed, execute the systemctl restart ocserv command to restart ocserv
