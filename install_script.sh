...
function check_root(){
	[[ $EUID != 0 ]] && echo -e "${Error} The current account is not ROOT (or does not have ROOT permissions),Cannot proceed,please use${Green_background_prefix} sudo su ${Font_color_suffix} To obtain temporary ROOT permissions (you will be prompted to enter the current account password after execution)." && exit 1
}
function check_sys(){
...
function sys_clean(){
	...
	sed -i '/#Automatic adjustment mtu，ocserv server use/d' /etc/rc.d/rc.local
...
#Generate certificate cancellation file
cd /root/iVPN/
touch /root/iVPN/revoked.pem
cd /root/iVPN/
cat << _EOF_ >crl.tmpl
crl_next_update = 365
crl_number = 1
...
cd /etc/ocserv/
rm -rf ocserv.conf
wget https://raw.githubusercontent.com/devsmine/ocserv/master/ocserv.conf
#
cd /root/iVPN
wget https://raw.githubusercontent.com/devsmine/ocserv/master/gen-client-cert.sh
wget https://raw.githubusercontent.com/devsmine/ocserv/master/user_add.sh
wget https://raw.githubusercontent.com/devsmine/ocserv/master/user_del.sh
...
EOF
echo "iVPN-ocserv server is installed, the service is ready to restart, and it can be used normally after restart"
reboot
}
...
check_sys
	if [[ ${release} == "centos" ]]; then
		...
		echo "Your operating system is not centos, please try again after changing the operating system"  && exit 1
	fi
}
shell_install
